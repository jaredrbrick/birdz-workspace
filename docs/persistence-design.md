...
4. ~~Remaining (birdzReact repo): on login, exchange the user-pool session for identity-pool credentials; load with one `Query`, save with `PutItem`/`UpdateItem`; keep localStorage as offline cache/fallback.~~ Done 2026-07-10 (birdzReact PR #5, `src/utils/cloudSave.ts`). Partition key is the Cognito **identity ID**, enforced by IAM. One deliberate deviation from the sort-key sketch above: the app saves a single `PROGRESS` item holding both `base` and `progress` (matching the app's atomic save unit) rather than separate `SIGHTING#` items — those remain available for future features.
