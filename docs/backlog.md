...
| P1 | AdSense (Google side) | Rejected "low value content" 2026-07-20; fixes (#32/#33 prerender + crawlability) **now live on prod (promoted 2026-07-30, Jared's go)** — birdzgame.com serves real HTML on /, /guide, /about, /privacy. **Next: Jared clicks "Request review" in AdSense**; ad placements wire up after approval |
| P1 | Region map — canonical, shared, travel-oriented | Jared 2026-07-30 (clarified): the point is that **users will physically go to different regions** — the game needs "the idea of a region map": a canonical division of the world into regions (eventually hundreds, bigger is fine) that is **consistent between people in places** — two players in the same spot always get the same region. Implications: (a) region assignment must be **deterministic from coordinates** against one shared region map — today's per-player fuzzy detection (Overpass/Nominatim confidence fallbacks + hand-tuned lat/lng boxes) can disagree between players; (b) an in-game **region map UI** (see where you've been / where your bases are / what's out there to travel to); (c) travel is the gameplay loop this feeds — pairs with GPS/photo-only placement (#37) and multiple bases. Needs a real region dataset (e.g. EPA/WWF ecoregions) + generated lookup, and roster depth per region. Design pass with Jared before building |
...
| P2 | S3 lifecycle rules — MOSTLY SHIPPED 2026-07-30 | Terraform written (30-day noncurrent expiry on site buckets + delete-marker cleanup + 7-day multipart abort; 365-day window on the state bucket), applied to **dev/test/staging** via CI. **Two remainders: (1) prod-branch promotion of the terraform was blocked by the permission classifier — needs Jared's go or a run of the promote himself; (2) bootstrap (state bucket) uses local state by design — Jared runs `terraform apply` in terraform/bootstrap once.** Audit aside still open: daily-use IAM user/role instead of the root user |
...
| P2 | Roster growth (post-74) | 74 birds after batch 8 (#41, 2026-07-30). **Batch-8 media staged in scripts/roster-batch8 — Jared runs the S3 upload** (BIRDZ_ENVS=dev,test,staging,prod node upload-media.mjs), app degrades gracefully until then. Thinnest pools now: mojave-desert (8; further growth needs silent adds — zero free audio exists for the whole wishlist, checked 2026-07-30), gulf-coast/pacific-northwest-forest worth a look next batch |
...
- **Roster batch 8 (#41): 67 → 74** — BTG Warbler, Hooded Warbler, Louisiana Waterthrush, Eastern Whip-poor-will, Wild Turkey, Tufted Titmouse (PD USFWS audio!), Rock Wren. Appalachians 9→15, mojave 7→8; all with verified commercially-safe audio + species-verified photos (caught the Wikipedia whip-poor-will lead image being a Fuertes painting). **PvP lambda ROSTER synced 60→74 — batch 7 had never entered the PvP deck** (drift caught). Live dev/test/staging (code); media upload = Jared; prod = Jared's go
- **Free plots (#39)**: SLOT_COSTS → all zero per Jared's correction ("create more bases immediately after starting… emphasize the travel aspect") — with GPS/photo-only placement, travel is the cost. Pricing mechanism kept for future tuning. Live dev/test/staging; **prod pending Jared's go**
- **Untrack .claude/ (#40, Jared's ask)**: verify-skill file removed from the birdzReact repo (kept on disk for local sessions), `.claude/` gitignored
- **PROD PROMOTED (Jared: "move to prod")** — all four envs converged on the same pin/bundle: prod picked up #32–#38 (prerender + crawlability, roster expansion to 67, difficulty curve, recent visitors, base cap 5, GPS-only placement, region tags) + the spa-router CloudFront function via prod Terraform. Verified identical bundle on all four envs over HTTP
- **Region-tag accuracy pass (#38) — live all envs**: appalachians 4→9, mojave-desert 4→7 by tagging existing in-range birds (wood_thrush, ovenbird, northern_cardinal, eastern_towhee, white_throated_sparrow; burrowing_owl, pinyon_jay, sage_thrasher). Zero new media. Batch-8 candidates researched with verified audio: docs/roster-batch8-candidates.md
- **Photo/GPS-only base placement (#37) — live dev/test/staging**: "Pick on map" tab retired per Jared (a base is a real place you've been; pairs with region consistency). MapPicker + leaflet dependency removed entirely; setup = photo dropzone + "Use my current location"; error copy updated; verify-skill doc now carries the headless geolocation recipe (CDP setGeolocationOverride)
