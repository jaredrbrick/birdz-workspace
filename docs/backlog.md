...
|----------|------|----------------|
| P1 | Trial/guest mode | **In flight** (branch `trial-mode`, 2026-07-11). Guest button on landing → play with localStorage-only saves; signup upsell banner; guest progress wins over empty cloud on signup |
| P1 | Bird data accuracy audit | Jared 2026-07-11: original entries were generated by a less sophisticated model — review all bird data (facts, phonetics, rarity, difficulty, biomes, regions, prefers) for accuracy and pertinence; fix in a PR. Oldest 24 entries first. Next after trial mode |
