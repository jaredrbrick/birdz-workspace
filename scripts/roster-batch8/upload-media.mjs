...
if (process.env.HTTPS_PROXY) {
  const caPath = process.env.BIRDZ_PROXY_CA || '/root/.ccr/ca-bundle.crt'
  handlerOpts.httpsAgent = new HttpsProxyAgent(process.env.HTTPS_PROXY, { ca: readFileSync(caPath) })
}

const ENVS = (process.env.BIRDZ_ENVS || 'dev,test,staging').split(',')
const BASE = new URL('./media', import.meta.url).pathname
const s3 = new S3Client({ region: 'us-east-1', requestHandler: new NodeHttpHandler(handlerOpts) })

const NEW_BIRDS = ['black_throated_green_warbler', 'hooded_warbler', 'louisiana_waterthrush',
  'eastern_whip_poor_will', 'wild_turkey', 'tufted_titmouse', 'rock_wren']
